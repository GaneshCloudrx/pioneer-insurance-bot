...
_REPORT_COLS = [
    "Timestamp", "API ID", "Message ID",
    "Patient Name", "Patient DOB", "Prescriber Name",
    "Drug Name", "Transferred NDC",
    "Pharmacy", "Rx Type",
    "Status", "Remark", "API Response",
]
    ...
    try:
        with open(path, "a", newline="", encoding="utf-8") as f:
            ...
            writer.writerow([
                ...
                tx.get("api_id", ""), tx.get("message_id", ""),
                tx.get("patient_name", ""), tx.get("patient_dob", ""),
                tx.get("prescriber_name", ""),
                tx.get("drug_name", ""), tx.get("ndc", ""),
                config.PHARMACY_NAME, config.PRESCRIPTION_TYPE,
                status, remark, tx.get("api_response", ""),
            ])
    except Exception as e:
...
def cleanup_old_recordings(days_old=2):
